...
class_prior_delta = int(sys.argv[3])
cond_prob_delta = float(sys.argv[4])
model_file = open(sys.argv[5], 'w')
sys_output = open(sys.argv[6], 'w')

#store training data
feat_counts = {} #{label : {word : count in docs with label}}
class_counts = {} #{label : total count of words in docs with label}
...
class_total = sum(class_counts.values()) #total number of words
class_num = len(class_counts) #number of classes
word_num = len(word_counts) #|V|
...
for c in feat_counts: #calculate conditional probabilities and unknown values and store logprobs in dictionaries
	...
	cond_probs[c] = {}
	unknown = cond_prob_delta / (class_counts[c] + (cond_prob_delta * word_num))
	unknowns[c] = math.log10(unknown)
	for word in feat_counts[c]:
		cond_prob = ((feat_counts[c][word] + cond_prob_delta)/(class_counts[c] + (cond_prob_delta * word_num))) 
